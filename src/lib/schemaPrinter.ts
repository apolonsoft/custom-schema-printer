/*
  Schema printer based on graphql 14.3.1 version (or so)
*/
...
function printDescription(
  ...
  let text = lines.join('\n');
  const isTypeExtension = text.startsWith(prefixTypeExtension);
...
function printFieldDirectives(field) {
  ...
  return directives == null ? "" : directives.map(d => ` @${d.name.value}${printFieldDirectiveArgs(d.arguments)}`);
}
